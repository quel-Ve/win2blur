...
LRESULT CALLBACK SettingsWndProc(HWND hwnd, UINT msg, WPARAM wp, LPARAM lp) {
    ...
    case WM_CREATE: {
        ...
        CreateWindowW(L"STATIC", L"Effect mode", WS_CHILD | WS_VISIBLE, 12, y + 8, 150, 20, hwnd, nullptr, hi, nullptr);
        ...
        hTintTrack = CreateWindowW(L"msctls_trackbar32", L"", WS_CHILD | WS_VISIBLE | TBS_AUTOTICKS | TBS_TOOLTIPS | TBS_NOTICKS, 12, y + 68, 200, 28, hwnd, (HMENU)110, hi, nullptr);
        SendMessageW(hTintTrack, TBM_SETRANGE, TRUE, MAKELONG(0, 100));
        SendMessageW(hTintTrack, TBM_SETPOS, TRUE, *p->pTint);
        wsprintfW(buf, L"Tint (black level): %d%%  (0 = pure blur)", *p->pTint); SetWindowTextW(hTintTitle, buf);
        y += 96;
    ...
    case WM_HSCROLL: {
        ...
        if (id == 101) { int v = (int)SendMessageW(hTrack, TBM_GETPOS, 0, 0); wchar_t b[64];
            ...
            wchar_t b[64]; wsprintfW(b, L"Tint (black level): %d%%  (0 = pure blur)", t);
    ...
    case WM_COMMAND:
        if (LOWORD(wp) == IDOK) {
            ...
            if (mode != g_effectMode || tint != g_tintLevel) {
                ...
                *p->pTint = tint; g_tintLevel = tint; write_int(L"TintLevel", tint);
                ...
                for (HWND h : affected) {
                    ...
                    int a = 255;
                    EnterCriticalSection(&g_fxLock);
                    if (g_winAlpha.count(h)) a = g_winAlpha[h];
                    LeaveCriticalSection(&g_fxLock);
...
void show_settings(HINSTANCE hInst) {
    ...
    int h = g_hasBlurControl ? 556 : 506;   // +96 for effect mode / tint block
