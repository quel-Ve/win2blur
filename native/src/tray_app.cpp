...
static std::map<HWND, int> g_winAlpha;    // v2.8 crisp mode: per-window opacity 0-255
...
static int g_effectMode = DEFAULT_EFFECT_MODE;   // forced Acrylic (DWM) — selector removed
static int g_tintLevel = DEFAULT_TINT_LEVEL;     // fixed 0 (pure blur) — slider removed
static HICON g_hIcon = nullptr;
// LL 键盘钩子：左右 Alt 可区分（VK_LMENU/VK_RMENU）——左 Alt=单窗口，右 Alt=全部 auto-frost
static HHOOK g_kbHook = nullptr;
static bool g_laltDown = false;
static bool g_raltDown = false;
static int g_swallowVk = 0;   // 已吞掉的按键（等其 keyup 一并吞）
...
void load_config() {
    // v2.8: 效果模式固定 Acrylic (DWM)，tint 固定 0（纯模糊）——selector/slider 已移除
    g_effectMode = DEFAULT_EFFECT_MODE;
    g_tintLevel = DEFAULT_TINT_LEVEL;
...
static bool is_elevated();   // defined below — used by the elevation gate
void detect_self_dll() {
    ...
    for (int i = 0; i < 4; i += 2) {
        // v2.8-maint: elevation gate — unelevated runs (manual launch) can't
        // inject (runas removed), so hide the radius slider there too
        // (logon-task runs are elevated and keep full control).
        if (is_elevated() &&
            GetFileAttributesW((dir + paths[i]).c_str()) != INVALID_FILE_ATTRIBUTES &&
...
void inject_self_dll() {
    // v2.8-maint: UAC-free — runas fallback removed. Not elevated (manual
    // launch outside the logon task) -> skip injection silently; the blur
    // radius slider is hidden by the elevation gate in detect_self_dll, so
    // the control never appears dead. Elevated path (logon task) injects
    // with no prompt.
...
void save_session(bool clear) {
    ...
    for (auto hwnd : g_modified) {
        ...
        int alpha = 255;
        if (g_winAlpha.count(hwnd)) alpha = g_winAlpha[hwnd];   // crisp: tracked value
        ...
        int effect = g_overlays.count(hwnd) ? g_effectMode + 1 : 0;
        wsprintfW(buf, L"%s|%s|%d|%d", title, cls, alpha, effect);
...
void restore_session() {
    ...
    for (auto& si : g_sessionItems) {
        ...
        EnterCriticalSection(&g_fxLock);
        g_modified.insert(h);
        g_winAlpha[h] = si.alpha;
        LeaveCriticalSection(&g_fxLock);
        if (si.tint != 2) {   // crisp windows are never made layered
        ...
        if (si.tint > 0) {   // effect: 1 = acrylic, 2 = crisp
            int mode = si.tint - 1;
            wchar_t cmd[512];
            if (mode == 1) {
                ...
                if (wash <= 0) continue;
                wsprintfW(cmd, L"\"%s\" 0x%08X %d %d", g_crispPath.c_str(),
                          (DWORD)(ULONG_PTR)h, wash, g_tintLevel);
            } else {
...
int current_alpha(HWND hwnd) {
    if (g_effectMode == 1) {
        EnterCriticalSection(&g_fxLock);
        int a = g_winAlpha.count(hwnd) ? g_winAlpha[hwnd] : 255;
        LeaveCriticalSection(&g_fxLock);
        return a;
    }
...
void prune_stale() {
    ...
    // or HIDDEN (closed-to-tray apps keep their HWND alive, e.g. cloudmusic).
    // Erase from g_modified/g_autoApplied too, so re-showing re-applies.
    for (auto it = g_overlays.begin(); it != g_overlays.end(); ) {
        HWND h = it->first;
        DWORD pid = 0; GetWindowThreadProcessId(h, &pid);
        bool recycled = (g_winPid.count(h) != 0 && pid != g_winPid[h]);
        if (!IsWindow(h) || recycled || !IsWindowVisible(h)) {
            TerminateProcess(it->second.hProcess, 0);
            CloseHandle(it->second.hProcess); CloseHandle(it->second.hThread);
            g_modified.erase(h); g_autoApplied.erase(h);
...
unsigned tint_argb() {
    ...
    int a = t * 255 / 100;
    // Acrylic accent renders NOTHING at alpha 0 (gradient alpha = effect
    // opacity) — floor at v2.7's 0x1A (10% black) so blur is always visible.
    if (a < 0x1A) a = 0x1A;
    return ((unsigned)a) << 24;   // black level
}
...
void launch_overlay(HWND hwnd) {
    ...
    if (g_winAlpha.count(hwnd)) alpha = g_winAlpha[hwnd];
    LeaveCriticalSection(&g_fxLock);
    if (dup) return;
    if (g_effectMode == 1) {
        if (g_crispPath.empty()) g_crispPath = extract_resource(103, L"crisp_overlay.exe");
        int wash = 255 - max(0, min(255, alpha));          // opacity -> wash strength
        if (wash <= 0) return;                              // fully opaque: no wash
        wsprintfW(cmd, L"\"%s\" 0x%08X %d %d", g_crispPath.c_str(),
                  (DWORD)(ULONG_PTR)hwnd, wash, g_tintLevel);
    } else {
        if (g_overlayPath.empty()) g_overlayPath = extract_resource(101, L"acrylic_overlay.exe");
        wsprintfW(cmd, L"\"%s\" 0x%08X 0x%08X", g_overlayPath.c_str(),
                  (DWORD)(ULONG_PTR)hwnd, tint_argb());
    }
    STARTUPINFOW si = {sizeof(si)}; si.dwFlags = STARTF_USESHOWWINDOW; si.wShowWindow = SW_HIDE;
    PROCESS_INFORMATION pi = {};
...
// ==================== Hotkeys (LL hook — left/right Alt) ====================
// 对单个窗口应用一个热键操作（透明度步进 / 开关 / blur toggle）
static void apply_hotkey(HWND hwnd, int id) {
    if (!hwnd) return;
...
// 右 Alt 批量：对 AutoFrost 列表中所有已打开（可见）窗口应用同一操作。
// 复用 autofrost 的匹配逻辑（exe + 可选类名），新开应用即使 monitor 尚未轮询也命中。
static void apply_hotkey_batch(int id) {
    AutoFrostConfig cfg;
    EnterCriticalSection(&g_fxLock);
    cfg = g_auto;                    // 快照列表
    LeaveCriticalSection(&g_fxLock);
    if (cfg.apps.empty()) return;
    std::pair<AutoFrostConfig, int> ctx = { cfg, id };   // 栈上快照，EnumWindows 同步回调安全
    EnumWindows([](HWND h, LPARAM lp) -> BOOL {
        if (!IsWindowVisible(h)) return TRUE;
        if (GetWindowLongW(h, GWL_EXSTYLE) & WS_EX_TOOLWINDOW) return TRUE;
        if (GetWindowTextLengthW(h) == 0) return TRUE;
        auto* p = (std::pair<AutoFrostConfig, int>*)lp;
        for (auto& app : p->first.apps)
            if (autofrost_match(app, h)) { apply_hotkey(h, p->second); break; }
        return TRUE;
    }, (LPARAM)&ctx);
}

// LL 钩子：区分左/右 Alt（VK_LMENU/VK_RMENU，非 VK_MENU）。
// 左 Alt+方向键 → 前台窗口；右 Alt+方向键 → 全部 auto-frost 窗口。
// 命中即吞键（return 1），keyup 同步吞掉避免应用收到悬空 keyup。
LRESULT CALLBACK ll_kb_proc(int code, WPARAM w, LPARAM lp) {
    if (code >= 0) {
        const KBDLLHOOKSTRUCT& k = *reinterpret_cast<KBDLLHOOKSTRUCT*>(lp);
        if (w == WM_KEYDOWN || w == WM_SYSKEYDOWN) {
            if (k.vkCode == VK_LMENU) { g_laltDown = true; return CallNextHookEx(g_kbHook, code, w, lp); }
            if (k.vkCode == VK_RMENU) { g_raltDown = true; return CallNextHookEx(g_kbHook, code, w, lp); }
            int id = 0;
            if (k.vkCode == VK_LEFT) id = ID_UP;
            else if (k.vkCode == VK_RIGHT) id = ID_DOWN;
            else if (k.vkCode == VK_UP) id = ID_TOGGLE;
            ...
            if (k.vkCode == VK_LMENU) { g_laltDown = false; return CallNextHookEx(g_kbHook, code, w, lp); }
            if (k.vkCode == VK_RMENU) { g_raltDown = false; return CallNextHookEx(g_kbHook, code, w, lp); }
            if (g_swallowVk && (int)k.vkCode == g_swallowVk) { g_swallowVk = 0; return 1; }  // swallow keyup
        }
    }
    return CallNextHookEx(g_kbHook, code, w, lp);
}

void install_hotkeys() {
    g_kbHook = SetWindowsHookExW(WH_KEYBOARD_LL, ll_kb_proc, GetModuleHandleW(nullptr), 0);
}
void uninstall_hotkeys() {
...
// Alpha slider label — Acrylic mode fixed, always window opacity
static void relabel_alpha(HWND hTitle, HWND hTrack) {
    if (!hTitle || !hTrack) return;
    int v = (int)SendMessageW(hTrack, TBM_GETPOS, 0, 0);
    wchar_t b[64];
    wsprintfW(b, L"Default transparency: %d%%", v);
...
LRESULT CALLBACK SettingsWndProc(HWND hwnd, UINT msg, WPARAM wp, LPARAM lp) {
    ...
    case WM_CREATE: {
        ...
        wchar_t buf[64];
        // Left column (x<200) holds controls; Apply/Close sit bottom-right
        ...
        // v2.8: effect-mode selector + tint slider removed (mode forced to
        // Acrylic DWM, tint fixed at 0 = pure blur) — Auto-frost follows.
        ...
        relabel_alpha(hAlphaTitle, hAlphaTrack);
        ...
        y += 226;
        CreateWindowW(L"STATIC", L"Hotkeys (hold ALT + arrow):\r\n← more transparent    → less transparent\r\n↑ effect ON / OFF     ↓ blur ON / OFF\r\nLeft ALT = this window\r\nRight ALT = all auto-frost windows", WS_CHILD | WS_VISIBLE, 12, y + 22, 270, 84, hwnd, nullptr, hi, nullptr);
        ...
        CreateWindowW(L"BUTTON", L"Apply", WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON, 136, y + 90, 60, 24, hwnd, (HMENU)IDOK, hi, nullptr);
        CreateWindowW(L"BUTTON", L"Close", WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON, 204, y + 90, 60, 24, hwnd, (HMENU)IDCANCEL, hi, nullptr);
    ...
    case WM_HSCROLL: {
        ...
        else if (id == 103 && hAlphaTrack) relabel_alpha(hAlphaTitle, hAlphaTrack);
        return 0;
    }
    case WM_COMMAND:
        if (LOWORD(wp) == IDOK) {
...
void show_settings(HINSTANCE hInst) {
    ...
    int h = g_hasBlurControl ? 500 : 450;   // taller bottom: clearer hotkey legend + Apply/Close row
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT msg, WPARAM wp, LPARAM lp) {
    ...
    case WM_CREATE: {
        ...
        if (!g_welcomePath.empty()) {
            ...
            if (pi.hProcess) {
                // v2.8-maint: bounded wait (5s) — an unclicked demo window must
                // not block startup forever (inject / session restore / the
                // autofrost thread all wait behind it; app looked dead).
                WaitForSingleObject(pi.hProcess, 5000);
                CloseHandle(pi.hProcess); CloseHandle(pi.hThread);
            }
        }
    ...
    case WM_HOTKEY: break;   // 不再使用 RegisterHotKey — LL 钩子直接调用 apply_hotkey
